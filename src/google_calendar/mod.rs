...
pub async fn get_next_week(google: &GoogleConfig, calendar_id: &str, days_into_future: &u8) -> Option<Events> {
    let from_date = Utc::now();
    let to_date = from_date + chrono::Duration::days((*days_into_future).into());
