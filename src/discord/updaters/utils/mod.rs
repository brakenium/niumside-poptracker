...
use poise::serenity_prelude::{
    ChannelId, CreateMessage, CreateScheduledEvent, EditScheduledEvent, GetMessages, Message,
    MessageId, ScheduledEvent,
};
...
pub async fn get_message_or_create_new(
    ...
    let empty_message = CreateMessage::new().content("a");
    let message = match message_id {
        Some(message_id) => {
            let get_messages = GetMessages::new().around(message_id).limit(1);
        ...
        None => guild_channel.send_message(ctx, empty_message).await?,
    };
...
pub async fn create_or_edit_event(
    ctx: &serenity::Context,
    calendar: &DiscordCalendarConfig,
    event: Option<&ScheduledEvent>,
    event_fields: &ToScheduleEventFields,
) -> Result<ScheduledEvent, discord::Error> {
    let location = event_fields
        .location
        .as_deref()
        .unwrap_or("Unable to get location");
    ...
    if let Some(event) = event {
        ...
        Ok(calendar
            .guild_id
            .edit_scheduled_event(ctx, event_id, edit_event)
            .await?)
    } else {
        let mut create_event =
            CreateScheduledEvent::new(External, &event_fields.title, event_fields.start_date_time)
                .end_time(event_fields.end_date_time)
                .location(location);
        ...
        Ok(calendar
            .guild_id
            .create_scheduled_event(ctx, create_event)
            .await?)
    }
}
