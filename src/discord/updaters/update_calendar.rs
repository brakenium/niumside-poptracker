...
use poise::serenity_prelude::{Colour, CreateEmbed, CreateMessage, EditMessage, FormattedTimestamp, GetMessages};
...
impl Updater for UpdateCalendar {
    async fn update(ctx: &serenity::Context) -> Result<(), discord::Error> {
        ...
        let events = google_calendar::get_next_week(&app_config.google, &app_config.discord.calendar.google_calendar_id).await;
        ...
        for event in events.items.unwrap_or_default() {
            let color_string = match google_calendar::get_event_color(&app_config.google, &event).await {
                Some(color) => color,
                None => {
                    get_calendar_color(&app_config.google, &app_config.discord.calendar.google_calendar_id).await.unwrap_or("000000".to_string())
                }
            };
            let color_int = u32::from_str_radix(&color_string[1..], 16).unwrap();
            let color = Colour::new(color_int);
